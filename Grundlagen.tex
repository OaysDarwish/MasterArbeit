...
\begin{figure}[H]
	...
	\caption{Einfache Darstellung des Regelkreises \textquotedblleft Fahrer-Fahrzeug-Umfeld\textquotedblright \citep{Appel2002}}
	\label{fig:FahrenRegelkreis}
\end{figure}

Die \autoref{fig:FahrenRegelkreis} zeigt einen vereinfachten Regelkreis des Verhaltens zwischen den drei Komponenten (Fahrer-Fahrzeug-Umfeld). In dem Modell wurde die Ablenkung als ein Störgrößenbeispiel an den Fahrer und die nasse Straße als eine Störgröße ans Fahrzeug. Dieses Modell macht es leichter den Ablauf eines Unfalls zu verstehen und anschließend weitere Unfälle zu vermeiden. Durch eine Störung an den Fahrer beziehungsweise ans Fahrzeug ändert sich der Ablauf einer Fahrt (oder eines Unfalls), da Störungen die Reaktionszeit sowie Reaktionsart der Fahrer stark beeinflussen. Diese sind im Fall eines Unfalls von großer Bedeutung. \\
...
Der Fahrstil „Drücken“ hat im Vergleicht zum Fahrstil „Legen“ keinen Vorteil im Zusammenhang des Grips auf der Straße. Es ist durchaus vorstellbar, dass mit zunehmender Schräglage die Reifenaufstandsfläche (beziehungsweise Reifenlatsch) abnimmt.
...
\begin{figure}[H]
	...
	\caption{Geschwindigkeitsüberschreitungen nach PKW- und Motorradfahrer an 7 Messstationen aus der VSS-Datenbank \citep{Maire2020}}
...
Die meisten Smartphones verwenden heutzutage Beschleunigungssensoren, um die Bildschirmanzeige abhängig von der Position auszurichten, in der das Gerät gehalten wird. Mit den eingebauten Beschleunigungssensoren können Benutzer ein besseres Anzeigeerlebnis erzielen. \citep{Sharma2020}
...
\end{itemize}
Im Allgemeinen ist der Beschleunigungssensor ein guter Sensor, wenn Sie die Bewegung des Geräts überwachen. \citep{DevelopersMotionSen}
...
\subsection{FFT} \label{abs:FFT}
\citep{NTIAudioFFT}\citep{WeissteinFFT}
...
\begin{figure}[H]
	...
	\caption{Beispiel von einem FFT (Zeitbereich und Frequenzbereich)\citep{NTIAudioFFT}}
...
In der \autoref{fig:AchsenSensor_Bike} ist der Unterschied zwischen den originalen Achsenrichtung (vom Smartphone) und diesen des Fahrrads anhand eines Fahrradbeispiel. In der Abbildung sind die Achsen des Smartphones mit den Ziffern 'SF' (Sensor frame) vermerkt, sowie diese des Fahrrads mit den Ziffern 'BF' (Bike Frame).
...
In der \autoref{fig:Entscheidungsbaum_Jan} ist der Entscheidungsbaum abgebildet, wonach eine Unfallklassifizierung erfolgt wird. Die Einzelkomponenten (CollisionHit, GroundHit, TipOver) werden später einzeln näher betrachtet.
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Bilder/Entscheidungsbaum_Jan.png}
	\caption{Entscheidungsbaum \citep{Schneeclassification2021}}
	\label{fig:Entscheidungsbaum_Jan}
\end{figure}

\subsection{TipOver} %TODO: umformulieren
%
%
%
%
...
\begin{align*}
	\lambda_{zul} \leq \arctan(1) \leq \ang{45}
\end{align*}

D.h. Bei einer Neigung von über \ang{45}, sollte das Motorrad umkippen. 
...
\subsection{GroundHit}
%
%
%
%

Dieses Modell dient dazu, einen Schlag zu erkennen, wenn das Motorrad am Boden ankommt. Dieses Modell geht von der Energie aus der \autoref{gl:EnergieFormel_JanPaper} aus. Dieses Modell hat einige Spezifikationen, die die verschiedene Szenarien abdeckt. Zwei typische Szenarien sind in der \autoref{fig:BicycleGroundHit2Angles} dargestellt. Die \autoref{fig:BicycleGroundHit2Angles1} zeigt der Fall, wenn ein Fahrrad nach einer ursprünglichen aufrechten Position umkippt, und die \autopageref{fig:BicycleGroundHit2Angles2} das Umkippen eines Fahrrads mit einer ursprünglichen Neigung (z.B. in einer Kurve).
Die Energie vom GroundHit im ersten Fall (\autoref{fig:BicycleGroundHit2Angles1}) ist deutlich größer, deswegen wird der Schwellwert nach dem Neigungswinkel angepasst. Je größer die Motorradneigung ist, desto kleiner ist der GroundHit-Schwellwert.\\
...
%	\hfill
	...
	\end{subfigure}
	\caption{Beispiel eines Fahrrads beim Umkippen mit einer aufrechten Postion und mit einer Neigung}
...
In der \autoref{fig:GH_Testfahrt_noGroundHit_FullView} und \autoref{fig:GH_Testfahrt_noGroundHit_EnergyZoomed} ist der Fall vorgestellt, in dem kein Groundhit erkannt wurde. Während dieser Fahrt fand kein Unfall statt. 
...
\subsection{CollisionHit} %TODO: Abschnitt erweitern
%Beispiel...
%\begin{table}
%	\centering
...
\begin{figure}[H]
	\centering
	\includegraphics[frame, width=\linewidth]{Bilder/CH_ID_2546_FullView.png}
	\caption{Crash mit CollisionHit - ID 2546 - Fullview}
	\label{fig:CH_ID_2546_FullView}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Bilder/CH_ID_2546_zoom.png}
	\caption{Crash mit CollisionHit - ID 2546 - gezoomt}
	\label{fig:CH_ID_2546_zoom}
\end{figure}


\section{Integrierte Aktivitätserkennung}
